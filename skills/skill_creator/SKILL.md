...
- The LLM judges skill quality. A vague skill description is a bug because selection depends on it.
- Do not ask the user to decide whether the skill is good as the default path. User override is an explicit action.
- Do not create hidden DB-only skills. Skill truth must be ordinary files.
- Generated skills land in `{storage_dir}/skills/generated/` and are immediately eligible (if required tools match). There is no draft or pending state.
