...
- 长期 memory 是 `internal/memorymodule` 的 file-backed `facts/`、`skills/`、`history/`。Canonical Memory Record V2 frontmatter 的 schema 包含 status/created/updated、tags、validity window、`source_run`、`source_refs`、`evidence_refs` 和 typed relations(`supports`、`derived_from`、`supersedes`、`contradicts`)。**fact 写入走结构化 `remember`/`CreateFact` 工具,由后端自动盖 status/created/updated/scope;tags 可选,validity/provenance/typed relations 是可选高级字段(默认不要求、不校验)**;raw `memory_create_file` 仍要求完整 frontmatter,作为高级/CLI 路径保留。skill/procedure 路径的 `origin`/`source_run`/`evidence_refs` 门槛不变(与 fact 解耦)。Search、Prepare、list 和 semantic projection 默认按 active records 工作;inactive/retired 只能通过显式 include 参数查看,不能由 client 自行推断。
- retrieval/eval truth 是 `memorymodule.Search/Prepare` 的可选 `SearchExplain`、source-backed replay fixtures、`source_ref_backlink` deterministic boost,以及必接入的 Bleve+FAISS semantic retrieval index。**不要引入 pgvector/PGLite、LanceDB 或第二套 retrieval store,也不要复活 never-wired 的 retrieval/skill eval sample schema + JSONL capture sink(已物理删除的投机性基建)**。
- semantic retrieval 配置 `memory.semantic`(独立 OpenAI-compatible embedding base_url/model/api_key/dimensions/timeout/batch_size + Bleve path/index_name)是**语义检索**运行的前提,但**惰性接线**:embedder/FAISS index 在首次 `Search`/`Prepare`/rebuild 时才构造,所以 `acorn pair`/`doctor`/serve 启动不被 embedding 配置或 FAISS 可用性阻塞。**语义检索是可选增强,不是发任务的前置闸门**:embedding 未配置时不接线语义运行时(`Config.MemorySemanticConfigured` 以 embedding model/base_url 是否存在判定),此时**显式记忆检索**(`Search`/`SearchSemantic`、memory 检索工具、`/v1/memory/search`、`acorn memory`)直接 fail-loud(`semantic search runtime is required`),而 **run 热路径的隐式 `Prepare` 降级为返回空 memory 结果(零召回是合法 baseline),让 run 照常进行**;`ValidateExecutionReady` 同理——未配置 embedding 不阻塞执行就绪,配置了才全字段校验。**不存在 `memory.semantic.enabled` 开关;语义运行时一旦接线就不静默降级,`Prepare`/`Search` 在已接线但 embed/index 调用失败时仍 fail-loud,也不 fallback 到关键词搜索或 fake vectors**(关键词检索路径已物理移除)——改变的只是「未接线」成为合法降级使用态,而非 run 闸门;`acorn memory semantic rebuild` 显式从 canonical Memory Record V2 records 重建 index,semantic text/hash/Bleve document 必须包含 v2 metadata。
...
- provider `api_key` 与 `memory.semantic.embedding.api_key` 都支持环境变量展开;embedding key 是独立 key,**不能从 chat provider key 静默复用**。chat provider key 缺失始终是 readiness/config error;embedding key 仅在**语义检索已配置时**(embedding model/base_url 存在)缺失才报 config error——未配置 embedding 时语义检索不接线、不阻塞 readiness(见「记忆 & 检索」)。已配置场景下缺密钥必须显式失败,不 fallback 到其他 provider 或 fake success(systemd path 通过 `~/.acorn/acorn.env` 的 `OPENAI_API_KEY` 注入)。
