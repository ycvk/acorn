...
- 当前 runtime 主链是 `Executor -> RunnerFactory/runBuilder -> ContextPlane + DecisionPlane + OrchestrationPlane -> ContextSession -> SQLite/file-backed memory`。
...
- 当前 native skill truth 是 `internal/skills` file-backed loader：`./skills` 是只读 builtin seed pack，generated skills 写入 `{runtime.storage_dir}/skills/generated`，workspace skills 写入 `./.acorn/skills/workspace`；skill lifecycle 通过 `skill_create`、`skill_assess` 和 `skill.lifecycle` RunEvent 维护。
...
- 当前 notification wake-up truth 是 SQLite `device_push_tokens`、`notifications`、`notification_deliveries` + `internal/notifications` APNs/FCM dispatcher ports；未配置 dispatcher 时 delivery status 必须是显式 `not_configured`，不能伪造 sent。push payload 只能是 wake-up metadata，不能带 tool output、memory、transcript、secret 或完整 run result。
...
- `./skills` 是 Acorn 自带 builtin seed pack，不是 ordinary writable workspace skill source。运行时不得用 memory/file tools 静默改 builtin skill；builtin 更新走代码变更。
...
- skill health / routing fixture / skill pack governance 是 deterministic 检查和 operator contract，不是 lifecycle promotion。pack install/update 只面向 workspace/generated/user mutable sources，必须 dry-run 可见、dependency closure 通过、receipt/hash 写入成功；builtin seed pack 仍只读。
...
- `configs/acorn.selfhosted.example.yaml` 是可提交自托管示例配置，必须保持 `~/.acorn` storage、`/srv/acorn/workspace` tool roots 和 `127.0.0.1:8080` default listen path 可用；systemd 设置 `HOME=/var/lib/acorn` 后实际 storage 是 `/var/lib/acorn/.acorn`。公网或私网暴露必须由 operator 显式改配置或反向代理完成。
- Provider `api_key` 支持环境变量展开；自托管 `systemd` path 通过 `/var/lib/acorn/.acorn/acorn.env` 的 `OPENAI_API_KEY` 注入。缺失密钥必须显式表现为 readiness/config error，不能 fallback 到其他 provider 或 fake success。
