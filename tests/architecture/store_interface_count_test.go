...
// CONSUMERS (internal/runtime + internal/wire) must consolidate to <=4.
...
func TestConsumerStoreInterfaceCount(t *testing.T) {
	...
	if count > maxConsumerStoreInterfaces {
		t.Fatalf("consumer-owned store interfaces (Store/Port/Repository/Ledger) in internal/runtime + internal/wire top-level must consolidate to <=%d; found %d", maxConsumerStoreInterfaces, count)
	}
}
