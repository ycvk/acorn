...
`.index/insights/` is an L1 retrieval-routing layer, not a second memory truth. Insight files act as probe cards: they match conceptual situations through tags, applies_when, title, and body, then resolve `source_refs` back to canonical L0 records in `facts/`, `skills/`, and `history/`. `Prepare` may use insight hits to rank resolved source records, but prompt entries still come from verified L0 records.

`Search` and `Prepare` can return optional `SearchExplain` metadata when callers set `Explain=true`. Explain records ranking stages, final item scores, and score contributions with source refs for direct memory records, external insight-index boosts, `.index/insights` source resolution, and typed relation contributions. Relation stages are deterministic ranking signals; they never replace canonical refs with prose, and unresolved relation targets fail search/rebuild instead of being skipped. This metadata is for tests, replay, and debug; ContextPlane does not parse it or inject it into model context. Indexed and filesystem search paths share fail-loud semantics for retrieval boost failures, and `.index/insights` source refs must still resolve to canonical L0 records.

Retrieval ranking includes deterministic source-backed boosts. `source_ref_backlink` resolves `source_refs` back to canonical L0 records, respects active selection, adds or boosts those targets, and records an explain contribution that names both the referring record and target ref. Relation boosts resolve typed relation targets and record `relation_<type>` contributions. Missing source refs or relation targets fail search instead of being skipped.
...
Retrieval capture is explicit opt-in. `internal/retrievaleval` defines the shared sample schema and JSONL sink; `memorymodule` can convert Search/Prepare results into samples, but no runtime path creates a sink or records samples by default. Capture writes return ordinary errors to the caller and are not best-effort background work.

Terminal finalization appends a compact Record V2-compatible history event through `memorymodule.AppendHistory`. Durable fact/learned-skill updates are done by the agent through `memory_read_file`, `memory_create_file`, `memory_replace_span`, and `memory_search_text`; there is no backend LLM distillation worker in the ordinary run path. Memory write helpers run explicit mutation planning before writes: create, replace, retire, noop duplicate, or reject invalid. The planner is a validation boundary for file writes, not a candidate inbox or automatic memory writer.
...
Skill retrieval capture uses the same explicit `internal/retrievaleval` sample schema. `internal/skills` can convert candidate retrieval results into capture samples, but the selector does not record samples unless a caller explicitly provides and invokes a sink.
...
`memory.semantic` is required configuration for the rebuildable semantic retrieval index, not current L0 memory truth. There is no enable switch, and config load does not open Bleve or call an embedding provider. Runtime composition opens the Bleve+FAISS index and constructs the embedder explicitly; if the binary was not built with Bleve+FAISS support, startup fails. Config validation requires an independent embedding API key after environment expansion; chat provider credentials are not reused as semantic embedding credentials. Unknown provider and invalid embedding numeric values are config errors.
