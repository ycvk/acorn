...
Production code may directly import `internal/store/sqlite` only from app composition root files that open and wire the adapter: `internal/app/container.go` and `internal/app/container_bootstrap.go`. `internal/architecture/store_boundary_test.go` scans production Go imports and fails if sqlite is imported elsewhere.
