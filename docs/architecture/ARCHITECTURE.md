...
- `internal/crystallization/` 管可选 auto-crystallization domain：成功 run 可在明确启用后生成 `memorymodule` procedure record，并通过 `crystallization.Service` 暴露给 runtime；SQLite insight index 只是 adapter-side cache，由 app composition root 创建 `internal/store/sqlite` adapter 后注入。
...
| **Tool lifecycle state** | `internal/contextplane/toollifecycle/` 管理的 run-scoped 工具可见性状态，记录 loaded tools、deferred tools 和 tool result references；执行前由 `SafeParallelToolsNode` 调 `OnToolCall` 校验，执行后写入 durable `ToolResultLedger`。 |
