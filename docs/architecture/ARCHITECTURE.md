...
| **CompactionEngine** | `internal/contextplane/compaction/` 的 proactive compact 执行所有者；生成 no-tool structured continuation summary、保护 preserved tail/tool pairs，并产出 `context.compressed` projection 所需的 token/summary metrics。 |
...
| **ContextBoundary** | `internal/model.ContextBoundary` 的 persisted compact boundary fact；SQLite `context_boundaries` 保存 root mode、trigger、covered/preserved refs、transcript ref、summary 和 token metrics，`context.compressed` 只投影 `boundary_id` 与展示字段。 |
...
| **Trace** | run events 的 persisted projection；`internal/stream` 持有 event-record -> StreamItem projection 和 trace summary，`internal/app.TraceService` 从 SQLite runs/events 构建 trace 和 resume status。RunDetail 的 trace summary 由已加载的 run event records 原地投影。 |
...
| **Run detail surface** | 当前 run deep dive 入口，只消费 `GET /v1/runs/{run_id}/detail` aggregate；只暴露 run、thread、mobile live event activity、trace summary 和 run artifacts，不由 client 散读 legacy endpoints，也不暴露 raw diagnostic event payload、plan DTO 或 runtime workbench 聚合。 |
...
- **Run detail 装配 fail-loud**：run/thread lookup、event projection、trace summary 和 artifact projection 失败时 endpoint 返回错误，不伪造 clean、ready、empty trace 或 synthetic artifacts。
...
- **Client v1 不复用 legacy StreamItem**：remote client 的 live stream fact 是 `/v1` mobile live `RunEvent` subset；runtime internal 可以继续使用 `runtime.StreamItem` 作为执行内部事件形态，但它不是 OpenAPI/generated 或 mobile app source contract。
