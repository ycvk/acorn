...
| **Post-compact rehydration helper** | `internal/contextplane` 的 concrete post-compact packet 计划器；从 compact 前 context envelopes、tool lifecycle state 和显式 plan/recent-file 输入恢复 working checkpoint、selected skill、skill catalog、memory、tool state 等 reference packets，packet 超过 token limit 时直接失败。 |
...
| **PlanStore** | runtime graph 使用的计划持久化接口，消费 `internal/model.Plan` / `PlanEvidence`；`internal/runtime/plan_store.go` 只负责 step evidence/backlink 追加和 store port 调用，不提供迁移兼容 alias。 |
