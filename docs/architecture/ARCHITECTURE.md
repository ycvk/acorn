...
| **Run detail surface** | 当前 run deep dive 入口，只消费 `GET /v1/runs/{run_id}/detail` aggregate；Workbench、trace、plan 和 diagnostic raw events 都作为 detail 投影，不由 client 散读 legacy endpoints。 |
