...
operator CLI / authenticated remote clients
  ...
  -> ContextPlane + DecisionPlane + OrchestrationPlane
...
| **RehydrationPlanner** | `internal/contextplane` 的 post-compact packet 计划器；从 compact 前 context envelopes、tool lifecycle state 和显式 plan/recent-file 输入恢复 working checkpoint、skill、memory、tool state 等 reference packets，packet 超过 token limit 时直接失败。 |
...
- [runtime-context-memory-decision.md](runtime-context-memory-decision.md)：ContextPlane、MemoryModule、DecisionPlane bridge、skill retrieval、tool lifecycle。
...
- **Tool result ledger 是工具结果事实**：tool result refs、arguments、side effects 和 evidence backlinks 以 SQLite `tool_results` 为准；当前 tool output 不经过字符数 compressor，过期 tool message 只替换为 durable `tool_result_ref`；workspace checkpoint / rollback projection 只从 ledger/workbench 读取，不能从 assistant prose 或 client local state 推断。
