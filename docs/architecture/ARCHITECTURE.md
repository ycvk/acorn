...
- `internal/model/` 保存跨包 runtime/domain records，例如 plan、evidence、run archive、session summary、run context snapshot 和 context boundary schema；这些类型不是 sqlite-owned contract。
- `internal/store/` 保存跨包 store-facing service records、ledger records 和 sentinel errors；SQLite adapter 通过这些 ports/records 持久化事实，不拥有 runtime domain schema。
- `internal/store/sqlite/` 是 SQLite adapter / persisted truth：sessions、runs、events、plans、checkpoints、archives、session summaries、context boundaries、tool results 和 artifacts 都从这里持久化或由 workspace inspection 补充。除 app composition root 外，production 代码不直接依赖 sqlite package；runtime、app services 和 MCP provider 通过 consumer-owned ports 消费持久化能力。长期 memory 的 active truth 是 `memorymodule` Record V2 文件；旧 SQLite memory/codeintel tables/readers 和 `acorn memory migrate` CLI 已删除，schema migration 会丢弃残留旧表。
- `internal/app/client_service.go` 和 `internal/web/handlers_client.go` 暴露 `/v1` Thread/Message/Run/RunEvent/RunDetail client surface；live `RunEvent` 从 existing `events` table 投影 mobile live subset，不新建 event bus 或 `run_events` 表。
...
| **ContextPlane** | `internal/contextplane` 的运行时上下文边界，负责 context assembly、budget、deferred load 和 tool lifecycle；`internal/contextplane/compaction/` 负责 compression handlers；初始上下文预算使用共享 token counter，不再用字符串 trim 丢 active context。 |
...
| **CompactionEngine** | `internal/contextplane/compaction/` 的 proactive compact 执行所有者；生成 no-tool structured continuation summary、保护 preserved tail/tool pairs，并产出 SQLite `context_boundaries` 所需的 token/summary metrics。 |
| **Post-compact rehydration helper** | `internal/contextplane/compaction/` 的 concrete post-compact packet 计划器；从 compact 前 context envelopes、tool lifecycle state 和显式 plan/recent-file 输入恢复 working checkpoint、selected skill、skill catalog、memory、tool state 等 reference packets，packet 超过 token limit 时直接失败。 |
| **ContextBoundary** | `internal/model.ContextBoundary` 的 persisted compact boundary fact；SQLite `context_boundaries` 保存 root mode、trigger、covered/preserved refs、transcript ref、summary 和 token metrics，不再维护 parallel compressed RunEvent。 |
...
| **ToolResultLedger** | `internal/store` ledger contract + SQLite `tool_results` 的工具结果事实层；保存 result ref、arguments、status、preview/full text、side-effect refs 和 evidence refs，是 context rehydration、plan evidence backlink、workspace checkpoint/rollback side-effect 和 artifact reference 的共同来源。 |
...
| **MemoryModule** | `internal/memorymodule` 的 file-backed memory 模块；管理 facts、skills、history、search、prepare、history append 和 memory file tools。Canonical Memory Record V2 metadata 包含 validity、provenance、typed relations、lifecycle timestamps 和 active selection。检索 truth 是 facts/skills/history 经 Bleve+FAISS 语义索引投影，无独立 insight 路由层。 |
...
| **Runtime diagnostics** | run events 的 backend-only projection；`internal/stream` 持有 runtime event write/projection helpers 和 root interrupt context projection，`internal/app.RunResumeService` 从 SQLite runs/events 推导 resume status。RunDetail 不暴露 diagnostic summary 或 raw diagnostic payload。 |
...
| **Internal StreamItem** | runtime internal event write shape；不是 remote client 的 live stream contract，不出现在 OpenAPI schemas，也不作为 app/web service interface 参数。 |
| **Client RunEvent** | `/v1` 的 client-facing live event envelope；mobile client 的 send/live stream path 只消费 mobile live subset：run lifecycle、assistant delta/message、terminal status、resume、elicitation/operator question 和 `decision_blocked`。它由 SQLite `events` table 投影，SSE `id` 是 `event_id`，`event` 是 `type`，`data` 是完整 `RunEvent` JSON；tool/memory/skill/procedure/subagent 事件是 diagnostics，不进入 live contract，plan/context/MCP/sampling trace events 已删除。 |
...
- [data-web-store.md](data-web-store.md)：SQLite truth、events/runs/plans/session/artifacts、remote client DTO/API responsibility、`/v1` RunEvent replay/follow。
...
- **Post-compact context 由 contextplane rehydration helper 恢复**：compact 后第一轮模型输入必须显式注入 rehydrated packets；packet 有 kind/source/token limit，超限直接失败，不从 workspace 扫描 recent files。
...
- **Run detail 装配 fail-loud**：run/thread lookup、event projection 和 artifact projection 失败时 endpoint 返回错误，不伪造 clean、ready、empty events 或 synthetic artifacts。
...
- **Client v1 不复用 legacy StreamItem**：remote client 的 live stream fact 是 `/v1` mobile live `RunEvent` subset；runtime internal 可以继续使用 `internal/stream.StreamItem` 作为执行内部事件写入形态，但它不是 OpenAPI/generated、mobile app、app service 或 web service contract。
