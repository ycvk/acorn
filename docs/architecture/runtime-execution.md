...
- `RunnerFactory.New` 在 `internal/runtime/runner.go` 中只保留入口委托；`internal/runtime/run.go` 的 `RunnerFactory.buildRun` 构建 `ActiveRunner`。主链是：创建 chat model；bootstrap MCP 并构建 run tool catalog；调用 `memory.Service.Prepare` 得到 file-backed prepared memory；assemble ContextPlane；委托给 `buildDirectResponse`。
