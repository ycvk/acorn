...
- Mobile does not maintain a tool-progress read model. The foreground live stream consumes assistant, run lifecycle, approval/resume, and issue signal events only; tool execution details remain backend-owned through terminal tool results, artifacts, and audit events.
