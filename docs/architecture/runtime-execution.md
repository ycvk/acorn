...
- Tool result truth is persisted separately from stream events. SQLite `tool_results` stores each tool call's durable `tool_result_ref`, run/session/call identity, arguments JSON, status, preview, full text, token estimate, side-effect refs, and evidence refs. Workspace mutation checkpoints and rollback results are projected through the same side-effect refs. The runtime writes this ledger from ContextPlane on every tool result, and plan evidence backlinks are appended against the same durable record.
