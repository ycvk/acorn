...
| **buildRun** | `internal/runtime/run.go` 的 per-run assembly 入口，按固定主链接 model、tool catalog、prepared memory、ContextPlane、direct_response assembly，返回 `ActiveRunner`。 |
...
| **ToolExecutionScheduler** | `internal/runtime/tooldispatch` 的共享调度核；通过 `ToolContract.Execution` 消费 parallel policy（read_only 并行 / serial 串行）。 |
