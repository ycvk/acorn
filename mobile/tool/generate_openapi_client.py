...
class AcornApiClient {
  ...
  Future<PendingActionDecision> decidePendingAction(String actionId, PendingActionDecisionRequest request) async {
...
class PendingActionOption {
  const PendingActionOption({required this.id, required this.label, this.description});
...
class RunDetail {
  ...
  factory RunDetail.fromJson(Map<String, dynamic> json) {
    ...
    return RunDetail(
      ...
      terminalSessions: _list(workbench['terminal_sessions'], RunTerminalSession.fromJson),
...
int? _nullableInt(Object? value) => value is int ? value : value is num ? value.toInt() : null;
