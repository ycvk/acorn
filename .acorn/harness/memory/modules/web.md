...
- Core logic: stable
- 已知问题: 无
- 最近改动: 2026-05-30 删除 public trace/debug surface：`RunDetail.trace`、`RunResult.trace_summary`、OpenAPI `TraceSummary` schema 和 `trace_debug` feature flag 已移除；RunDetail 只暴露 run/thread、mobile live events 和 top-level artifacts。2026-05-29 `/v1/runs/{run_id}/events` hard-cut 为 mobile live subset；diagnostic-only persisted events 不再进入 SSE live contract、RunDetail raw payload、runtime workbench 或 plan DTO。
