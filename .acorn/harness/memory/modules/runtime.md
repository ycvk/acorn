...
6. Runtime diagnostic events（MCP/sampling/skill/procedure/memory/context/plan/subagent 等）可以持久化为 backend-only audit/projection source，但不能自动提升为 mobile live RunEvent，也不能作为 diagnostic summary、raw payload、public plan DTO 或 runtime workbench 聚合暴露到 `/v1` RunDetail；per-chunk tool progress 不持久化为 RunEvent；新增 live event kind 必须同步 `clientevents.IsLiveRunEventKind`、OpenAPI、generated mobile client 和 parser/projection tests。
