...
- Core logic: stable
- 已知问题: 无
- 最近改动: 2026-05-29 将 mobile live RunEvent contract 从 runtime diagnostic trace 中 hard-cut 出来；持久化 runtime events 仍是 SQLite trace truth，但 `/v1/runs/{run_id}/events` 只投影移动端 live 子集。同轮将重复的 root-runtime / `runtime/api` tool argument validator hard-cut 为 `internal/runtime/tool` 内部实现，`runtime/api` 只保留跨 runtime 子包共享的 context/plan/serializer contracts。随后删除 stale `SessionStateService` / `RuntimeWorkbenchService`、public RunDetail workbench/plan DTO surface 和 runtime `prepareSkillExecution` no-op seam；client-visible trace summary 统一委托 `internal/stream`。
...
6. Runtime diagnostic events（tool/MCP/sampling/skill/procedure/memory/context/plan/subagent 等）可以持久化并进入 backend trace summary，但不能自动提升为 mobile live RunEvent，也不能作为 raw payload、public plan DTO 或 runtime workbench 聚合暴露到 `/v1` RunDetail；新增 live event kind 必须同步 `clientevents.IsLiveRunEventKind`、OpenAPI、generated mobile client 和 parser/projection tests。
