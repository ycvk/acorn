...
func (s *Server) handleSearchMemory(w http.ResponseWriter, r *http.Request) {
	...
	result, err := s.memory.Search(r.Context(), memory.SearchRequest{
...
func parseMemoryKinds(raw string) ([]memory.Kind, error) {
	...
	kinds := make([]memory.Kind, 0, len(parts))
	for _, part := range parts {
		trimmed := strings.TrimSpace(part)
		switch kind := memory.Kind(trimmed); kind {
		case "":
		case memory.KindFact, memory.KindSkill, memory.KindHistory:
...
func parseRecordSelection(r *http.Request) (memory.RecordSelection, error) {
	includeInactive, err := parseBoolQuery(r, "include_inactive")
	if err != nil {
		return memory.RecordSelection{}, err
	}
	includeRetired, err := parseBoolQuery(r, "include_retired")
	if err != nil {
		return memory.RecordSelection{}, err
	}
	return memory.RecordSelection{
...
func limitRecords(items []memory.Record, limit int) []memory.Record {
