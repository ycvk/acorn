...
func TestSanitizeMCPProviderName(t *testing.T) {
	tests := []struct {
		...
		{"alphanumeric", "github", "github"},
		{"with dash", "notion-mcp", "notion-mcp"},
		{"with spaces", "My Server", "My-Server"},
		{"empty", "", "provider"},
		{"special chars", "a!@#b", "a---b"},
		{"only special", "!@#", "provider"},
	}
	for _, tt := range tests {
...
func TestAugmentDescription(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			desc: "Search for issues",
			want: "Search for issues\n\nProvided by MCP server: github",
		},
		{
			name: "empty description",
			desc: "",
			want: "Provided by MCP server: github",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := augmentDescription(tt.desc, "github")
			if got != tt.want {
				t.Errorf("augmentDescription(%q, ...) = %q, want %q", tt.desc, got, tt.want)
			}
		})
	}
}
...
func buildCapabilityRegistryForTest(
	...
	specs := make([]core.ToolSpec, 0, len(localTools)+len(registrations)+len(resourceTools)+len(promptTools))
	for _, tool := range localTools {
		specs = append(specs, core.ToolSpec{
			ToolContract: toolNamingContract("", "local", core.ToolKindNative, core.ToolCategoryRead, core.EagerLoadingPolicy()),
			Tool:         tool,
		})
	}
	for _, registration := range registrations {
		...
		specs = append(specs, core.ToolSpec{
			ToolContract: toolNamingContract("", registration.ProviderName, core.ToolKindMCP, core.ToolCategoryIntegration, core.EagerLoadingPolicy()),
			Tool:         namespaced,
		})
	}
	for _, tool := range resourceTools {
		...
		specs = append(specs, core.ToolSpec{
			ToolContract: toolNamingContract("", info.Name, core.ToolKindMCP, core.ToolCategoryIntegration, core.DeferredLoadingPolicy("deferred_mcp_catalog")),
			Tool:         tool,
		})
	}
