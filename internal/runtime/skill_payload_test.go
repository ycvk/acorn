...
func mustProjectStreamItemToEvent(t *testing.T, item core.StreamItem) (string, any) {
	t.Helper()
...
// produces after re-encoding the *core.StreamSkill via JSON.
...
func TestProjectStreamItemToEventProjectsSkillPayload(t *testing.T) {
	kind, payload := mustProjectStreamItemToEvent(t, core.StreamItem{
		RunID: "run_5",
		Kind:  core.StreamKindSkillSelected,
		Payload: map[string]any{"skill": &core.StreamSkill{
			...
			Requirements: core.StreamSkillRequirements{Tools: []string{"read_file", "run_command"}},
			Score:        145,
		}},
	})
...
func TestProjectStreamItemToEventProjectsNoSelectionReason(t *testing.T) {
	kind, payload := mustProjectStreamItemToEvent(t, core.StreamItem{
		RunID: "run_5b",
		Kind:  core.StreamKindSkillDiscovered,
		Payload: map[string]any{"skill": &core.StreamSkill{
			NoSelectionReason: "no_eligible_match",
			Candidates: []core.StreamSkillCandidate{
