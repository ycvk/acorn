...
func (s *directResponseTestStreamer) StreamAssistantMessage(ctx context.Context, req core.AssistantStreamRequest) (*core.AssistantStreamResult, error) {
...
func (s *directResponseTestStreamer) StreamAssistantInterleaved(ctx context.Context, req core.AssistantStreamRequest) *core.InterleavedStream {
	interleaved := &core.InterleavedStream{
...
func directResponseTestStreamResult(msg *schema.Message) *core.AssistantStreamResult {
	raw := ""
	if msg != nil && msg.ResponseMeta != nil {
		raw = strings.TrimSpace(strings.ToLower(msg.ResponseMeta.FinishReason))
	}
	stopReason := core.AssistantStopReasonEndTurn
	switch raw {
	case "tool_calls", "tool_use":
		stopReason = core.AssistantStopReasonToolCalls
	case "length", "max_tokens", "max_output_tokens", "model_context_window_exceeded":
		stopReason = core.AssistantStopReasonMaxOutput
	case "", "stop", "end_turn", "null":
		if msg != nil && len(msg.ToolCalls) > 0 {
			stopReason = core.AssistantStopReasonToolCalls
		}
	default:
		stopReason = core.AssistantStopReasonUnknown
	}
...
func (n *directResponseTestToolNode) NewStreamingExecutor(ctx context.Context) dispatch.StreamingExecutor {
...
func TestBuildDirectResponseContinuesAfterOutputLimit(t *testing.T) {
	...
	deps := RuntimeDeps{
		...
		ToolNodeFactory: func(context.Context, []einotool.BaseTool, core.ExecutionPolicyResolver) (dispatch.ToolInvoker, error) {
...
func directResponseCatalogForTest(t *testing.T, ctx context.Context, tool einotool.BaseTool) *tools.Catalog {
	...
	catalog, err := tools.NewCatalog(ctx, []core.ToolSpec{{
...
func directResponseContextResultForTest(runID string, sessionID string, eagerTools ...string) *AssembleResult {
	...
	for _, name := range eagerTools {
		...
		loaded[trimmed] = LoadedToolRecord{
			Name:       trimmed,
			LoadedAt:   now,
			LoadSource: "eager",
		}
...
func newDirectResponseTestSession(t *testing.T, ctx context.Context, assembly *RunAssembly) Session {
	t.Helper()
	...
	_, err = session.Bootstrap(ctx, BootstrapRequest{
		SessionID:       "session",
		RunID:           "run",
...
func (n *directResponseInterruptToolNode) NewStreamingExecutor(ctx context.Context) dispatch.StreamingExecutor {
...
func (n *directResponseApprovalToolNode) NewStreamingExecutor(ctx context.Context) dispatch.StreamingExecutor {
...
func TestBuildDirectResponseResumeContinuesFromPendingToolCalls(t *testing.T) {
	ctx := context.Background()
	RegisterTypes() // gob-register DirectResponseInterruptData for checkpoint serialization
	...
	session := newDirectResponseTestSession(t, ctx, assembly)
	runCtx := WithSession(ctx, session)
	...
	resumeCtx := WithSession(context.Background(), session)
