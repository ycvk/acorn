...
func (a *directResponseAgent) runFromState(ctx context.Context, generator *adk.AsyncGenerator[*adk.AgentEvent], resumeData *DirectResponseInterruptData) {
	...
	session := SessionFromContext(runCtx)
	if session == nil {
	...
	for iteration := startIteration; iteration < a.maxIterations; iteration++ {
		toolInfos := LoadedToolInfosFromContext(runCtx, a.eagerToolNames)
		messageID := directResponseMessageID(a.runID, iteration)
		modelInput, err := session.BeforeModelCall(runCtx, ModelCallRequest{
			CallID:    messageID,
			ToolInfos: toolInfos,
		})
		if err != nil {
...
func (a *directResponseAgent) resumePendingToolCalls(ctx context.Context, session Session, resumeData *DirectResponseInterruptData) error {
	if resumeData == nil {
