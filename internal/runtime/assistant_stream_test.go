...
func (s *assistantStreamAppenderSpy) AppendEvent(_ context.Context, runID, kind string, payload any) (core.EventRecord, error) {
...
func TestStreamAssistantMessageEmitsDeltaItemsAndReturnsFinalMessage(t *testing.T) {
	...
	if result.StopReason != core.AssistantStopReasonEndTurn {
		t.Fatalf("stop reason = %q, want end_turn", result.StopReason)
	}
	if len(items) != 2 {
		t.Fatalf("delta item count = %d, want 2", len(items))
	}
	if items[0].Kind != core.StreamKindAssistantDelta || items[1].Kind != core.StreamKindAssistantDelta {
...
func TestStreamAssistantMessageAppendsDeltaWithoutLiveSink(t *testing.T) {
	...
	delta := core.ItemGetAssistantDelta(item)
	if item.Kind != core.StreamKindAssistantDelta || delta == nil {
...
func TestDirectAssistantStreamerPersistsAndSinksDeltas(t *testing.T) {
	...
	var sinkItems []core.StreamItem
	ctx := core.WithStreamSink(context.Background(), func(item core.StreamItem) error {
	...
	first := core.ItemGetAssistantDelta(projectEventToStreamItem(appender.records[0]))
	second := core.ItemGetAssistantDelta(projectEventToStreamItem(appender.records[1]))
	if first == nil || second == nil {
...
func orchestrationAssistantStreamRequestForTest(runID string, model *assistantStreamingModel, toolInfo *schema.ToolInfo) core.AssistantStreamRequest {
	return core.AssistantStreamRequest{
		RunID:     runID,
...
func TestRunStateApplyStreamItemAppendsAssistantDelta(t *testing.T) {
	var state drRunState
	state.applyStreamItem(core.StreamItem{
		Kind: core.StreamKindAssistantDelta,
		Payload: map[string]any{"assistant_delta": &core.StreamAssistantDelta{
			Delta:    "partial ",
			Sequence: 1,
		}},
	})
...
func projectEventToStreamItem(event core.EventRecord) core.StreamItem {
	item := core.StreamItem{
		RunID:     event.RunID,
		Kind:      core.StreamItemKind(event.Kind),
...
type drRunState struct {
	lastOutput string
}

func (s *drRunState) applyStreamItem(item core.StreamItem) {
