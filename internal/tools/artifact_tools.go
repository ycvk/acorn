...
func buildArtifactTools(service *artifacts.Service, bridge ArtifactContext) ([]einotool.BaseTool, error) {
	if service == nil {
...
func buildArtifactWriteTool(service *artifacts.Service, bridge ArtifactContext) (einotool.BaseTool, error) {
	tool, err := inferProgressTool("artifact_write", "Persist run-scoped artifact content and return an opaque artifact id.", func(ctx context.Context, input ArtifactWriteInput, emit tooling.ToolProgressEmitter) (ArtifactWriteOutput, error) {
		...
		record, err := service.Write(ctx, artifacts.WriteRequest{
			...
			Kind:                artifacts.Kind(strings.TrimSpace(input.Kind)),
			Title:               input.Title,
...
func buildArtifactReadTool(service *artifacts.Service) (einotool.BaseTool, error) {
	tool, err := inferProgressTool("artifact_read", "Read an explicit byte range from a persisted artifact.", func(ctx context.Context, input ArtifactReadInput, emit tooling.ToolProgressEmitter) (ArtifactReadOutput, error) {
		result, err := service.ReadRange(ctx, artifacts.ReadRangeRequest{
...
func buildArtifactListTool(service *artifacts.Service, bridge ArtifactContext) (einotool.BaseTool, error) {
...
func artifactSummaryFromRecord(record artifacts.Record) ArtifactSummary {
