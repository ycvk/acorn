...
func buildWebSearchTool(search WebSearchService, artifactService ArtifactService, bridge ArtifactContext) (einotool.BaseTool, error) {
	...
	tool, err := inferProgressTool("web_search", "Search public web sources through the configured provider and persist the raw provider response.", func(ctx context.Context, input WebSearchInput, emit tooling.ToolProgressEmitter) (WebSearchOutput, error) {
