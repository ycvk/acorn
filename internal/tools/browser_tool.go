...
func buildBrowserTool(service BrowserService, artifactService ArtifactService, bridge ArtifactContext) (einotool.BaseTool, error) {
	...
	tool, err := inferProgressTool("browser", "Operate one run-scoped backend-owned Chromium session through fixed actions and persist explicit page/screenshot artifacts.", func(ctx context.Context, input BrowserInput, emit tooling.ToolProgressEmitter) (BrowserOutput, error) {
