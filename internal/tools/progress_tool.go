...
type progressToolFunc[I, O any] func(ctx context.Context, input I, emit tooling.ToolProgressEmitter) (O, error)
...
func (t *localProgressTool[I, O]) InvokableRunWithProgress(ctx context.Context, argumentsInJSON string, emit tooling.ToolProgressEmitter, _ ...einotool.Option) (string, error) {
	var input I
...
func emitToolProgress(ctx context.Context, emit tooling.ToolProgressEmitter, delta string) error {
	if emit == nil || delta == "" {
		return nil
	}
	return emit(ctx, tooling.ToolProgressEvent{Delta: delta})
}
