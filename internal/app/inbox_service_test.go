...
func TestInboxServiceLoadProjectsMobileAggregate(t *testing.T) {
	...
	if err := store.FinishRun(ctx, "run_terminal", domain.RunStatusSucceeded, "done", ""); err != nil {
...
func TestInboxServiceFailsOnInvalidPendingActionPayload(t *testing.T) {
	...
	if err := store.CreateRun(ctx, domain.RunCreateParams{RunID: "run_invalid_payload", SessionID: "thread_invalid_payload", TurnIndex: 1, Input: "work"}); err != nil {
...
func TestInboxServiceProjectsOperatorQuestionPendingAction(t *testing.T) {
	...
	if err := store.CreateRun(ctx, domain.RunCreateParams{RunID: "run_operator_pending", SessionID: "thread_operator_pending", TurnIndex: 1, Input: "ask"}); err != nil {
		t.Fatalf("create run: %v", err)
	}
	if _, err := store.CreatePendingAction(ctx, domain.PendingActionInput{
