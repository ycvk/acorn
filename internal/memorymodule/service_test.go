...
func TestSearchInsightsResolvesSources(t *testing.T) {
	service := newTestService(t)
	writeFile(t, service, "facts/workspaces/context.md", `---
scope: workspace:acorn
...
# Context Boundary Truth

SQLite context_boundaries are durable compact truth.
`)
	writeFile(t, service, ".index/insights/context-boundary.md", `---
id: acorn-context-boundary-truth
kind: insight
scope: workspace:acorn
status: verified
title: Acorn context boundary truth
tags: [resume]
applies_when:
  - deciding resume truth
source_refs:
  - facts/workspaces/context.md#context-boundary-truth
updated: 2026-05-08
---

Use this when discussing resume truth.
`)
	result, err := service.searchInsights(t.Context(), insightSearchRequest{
		Query: "resume truth",
		Scope: "workspace:acorn",
		Limit: 10,
	})
	...
	if len(result.Hits[0].Sources) != 1 {
		t.Fatalf("len(sources) = %d, want 1", len(result.Hits[0].Sources))
	}
	if result.Hits[0].Sources[0].Ref != "facts/workspaces/context.md#context-boundary-truth" {
		t.Fatalf("source ref = %q", result.Hits[0].Sources[0].Ref)
	}
}

func TestPrepareUsesInsightSourcesForConceptQuery(t *testing.T) {
	service := newTestService(t)
	writeFile(t, service, "facts/workspaces/context.md", `---
scope: workspace:acorn
tags: [context, boundary]
...
`)
	...
	result, err := service.Prepare(t.Context(), PrepareRequest{
		WorkspaceSlug: "acorn",
		UserInput:     "what is resume truth",
	})
	if err != nil {
		t.Fatalf("Prepare: %v", err)
	}
	...
	if !strings.Contains(result.Entries[0].Content, "context_boundaries") {
		t.Fatalf("entry content = %q", result.Entries[0].Content)
	}
}

func TestPrepareExplainIncludesInsightSourceContribution(t *testing.T) {
	service := newTestService(t)
	writeFile(t, service, "facts/workspaces/context.md", `---
scope: workspace:acorn
tags: [context, boundary]
...
`)
	...
	if result.Explain == nil {
		t.Fatal("Explain is nil")
	}
	if len(result.Entries) != 1 {
		t.Fatalf("len(entries) = %d, want 1: %#v", len(result.Entries), result.Entries)
	}
	ref := "facts/workspaces/context.md#context-boundary-truth"
	if !explainHasStage(result.Explain, searchStageInsightSource) {
		t.Fatalf("explain stages = %#v, want insight source stage", result.Explain.Stages)
	}
	if !explainHasContributionStage(result.Explain, ref, searchStageInsightSource) {
		t.Fatalf("explain missing insight source contribution for %s: %#v", ref, result.Explain.Items)
	}
	if !explainHasContributionSource(result.Explain, ref, ref) {
		t.Fatalf("explain missing L0 source ref %s: %#v", ref, result.Explain.Items)
	}
}

func TestSearchInsightsFailsWhenIndexedSourceDisappears(t *testing.T) {
	service := newTestService(t)
	writeFile(t, service, "facts/workspaces/context.md", `---
scope: workspace:acorn
tags: [context, boundary]
...
Use this when discussing resume truth.
`)
	if err := os.Remove(filepath.Join(service.Root(), "facts", "workspaces", "context.md")); err != nil {
		t.Fatalf("Remove: %v", err)
	}
	_, err := service.searchInsights(t.Context(), insightSearchRequest{
		Query: "resume truth",
		Scope: "workspace:acorn",
		Limit: 10,
	})
	if err == nil || !strings.Contains(err.Error(), "load indexed memory record") {
		t.Fatalf("searchInsights error = %v, want indexed record read failure", err)
	}
}

func TestPrepareRejectsInvalidInsightFrontmatter(t *testing.T) {
	service := newTestService(t)
	writeFile(t, service, ".index/insights/bad.md", `---
id: bad
kind: fact
status: verified
title: Bad Insight
...
`)
	setSemanticSearchHits(t, service, nil)
	_, err := service.Prepare(t.Context(), PrepareRequest{UserInput: "resume"})
	if err == nil || !strings.Contains(err.Error(), "insight kind must be insight") {
		t.Fatalf("Prepare error = %v, want insight kind validation", err)
	}
}

func TestPrepareRejectsInsightMissingTitle(t *testing.T) {
	service := newTestService(t)
	writeFile(t, service, ".index/insights/missing-title.md", `---
id: missing-title
kind: insight
status: verified
tags: [resume]
...
`)
	setSemanticSearchHits(t, service, nil)
	_, err := service.Prepare(t.Context(), PrepareRequest{UserInput: "resume"})
	if err == nil || !strings.Contains(err.Error(), "insight title is required") {
		t.Fatalf("Prepare error = %v, want missing title validation", err)
	}
}

func TestPrepareRejectsVerifiedInsightWithoutSourceRefs(t *testing.T) {
	service := newTestService(t)
	writeFile(t, service, ".index/insights/no-source-refs.md", `---
id: no-source-refs
kind: insight
status: verified
...
`)
	setSemanticSearchHits(t, service, nil)
	_, err := service.Prepare(t.Context(), PrepareRequest{UserInput: "resume"})
	if err == nil || !strings.Contains(err.Error(), "verified insight source_refs are required") {
		t.Fatalf("Prepare error = %v, want source_refs validation", err)
	}
}

func TestPrepareRejectsMissingInsightSourceRef(t *testing.T) {
	service := newTestService(t)
	writeFile(t, service, ".index/insights/missing-source.md", `---
id: missing-source
kind: insight
status: verified
...
tags: [resume]
source_refs:
  - facts/workspaces/missing.md#missing
updated: 2026-05-08
---
...
`)
	setSemanticSearchHits(t, service, nil)
	_, err := service.Prepare(t.Context(), PrepareRequest{UserInput: "resume"})
	if err == nil || !strings.Contains(err.Error(), "memory record not found") {
		t.Fatalf("Prepare error = %v, want missing source validation", err)
	}
}

func TestUnverifiedInsightDoesNotInjectUnverifiedSource(t *testing.T) {
	service := newTestService(t)
	writeFile(t, service, "facts/workspaces/context.md", `---
scope: workspace:acorn
tags: [context, boundary]
...
`)
	writeFile(t, service, ".index/insights/context-boundary.md", `---
id: acorn-context-boundary-draft
kind: insight
scope: workspace:acorn
status: unverified
title: Acorn context boundary draft
tags: [resume]
source_refs:
  - facts/workspaces/context.md#context-boundary-draft
updated: 2026-05-08
---
...
`)
	setSemanticSearchHits(t, service, []SemanticHit{{
		Ref:   "facts/workspaces/context.md#context-boundary-draft",
		Kind:  KindFact,
		Score: 4,
		Stage: searchStageSemanticHybrid,
	}})
	...
	if len(result.Nudges) != 1 {
		t.Fatalf("len(nudges) = %d, want 1", len(result.Nudges))
	}
	if len(result.Entries) != 0 {
		t.Fatalf("len(entries) = %d, want 0: %#v", len(result.Entries), result.Entries)
	}
...
func TestPrepareMergesDirectAndInsightMatches(t *testing.T) {
	service := newTestService(t)
	writeFile(t, service, "facts/workspaces/context.md", `---
scope: workspace:acorn
tags: [context, boundary]
...
Use context_boundaries for resume truth.
`)
	writeFile(t, service, ".index/insights/context-boundary.md", `---
id: acorn-context-boundary-truth
kind: insight
...
`)
	...
	result, err := service.Prepare(t.Context(), PrepareRequest{
		WorkspaceSlug: "acorn",
		UserInput:     "resume context_boundaries",
	})
	if err != nil {
		t.Fatalf("Prepare: %v", err)
	}
...
func TestPrepareSkipsInsightSourcesOutsideWorkspaceScope(t *testing.T) {
	service := newTestService(t)
	writeFile(t, service, "facts/workspaces/other.md", `---
...
`)
	writeFile(t, service, ".index/insights/global-context.md", `---
id: global-context
kind: insight
status: verified
title: Global context probe
tags: [resume]
source_refs:
  - facts/workspaces/other.md#other-workspace-context
updated: 2026-05-08
---
...
`)
	...
	if len(result.Nudges) != 0 {
		t.Fatalf("len(nudges) = %d, want 0: %#v", len(result.Nudges), result.Nudges)
	}
	if len(result.Entries) != 0 {
		t.Fatalf("len(entries) = %d, want 0: %#v", len(result.Entries), result.Entries)
	}
...
func explainHasContributionStage(explain *SearchExplain, ref string, stage string) bool {
	if explain == nil {
		return false
	}
...
func explainHasContributionSource(explain *SearchExplain, ref string, sourceRef string) bool {
	if explain == nil {
		return false
	}
	for _, item := range explain.Items {
		...
		for _, contribution := range item.Contributions {
			for _, got := range contribution.SourceRefs {
				if got == sourceRef {
					return true
				}
			}
