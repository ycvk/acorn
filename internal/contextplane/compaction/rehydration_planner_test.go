...
func TestDefaultRehydrationPlannerBuildsToolPlanAndRecentPackets(t *testing.T) {
	...
	plan, err := engine.buildRehydratePlan(context.Background(), RehydrateRequest{
		...
		ToolState: &contextplane.ToolLifecycleState{
			...
			RecentResults: []contextplane.ToolResultRecord{{
				CallID:    "call_1",
				ToolName:  "read_file",
				TurnIndex: 3,
...
func TestBuildRehydrateMessagesWrapsPacketsAsReferenceContext(t *testing.T) {
	...
	for _, want := range []string{"<rehydrate-packet>", contextplane.ReferenceContextNote, "Kind: working_checkpoint", "Source: test", "checkpoint content"} {
