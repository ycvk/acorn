...
type RehydrateRequest struct {
	...
	TokenBudget        int
	TokenCounter       contextplane.TokenCounter
}
...
func BuildRehydrateMessages(plan *RehydratePlan) []adk.Message {
	...
	for _, packet := range plan.Packets {
		content := strings.Join([]string{
			"<rehydrate-packet>",
			contextplane.ReferenceContextNote,
...
func extractPreparedMemoryPacket(memoryContext string) string {
	...
	trimmed = strings.ReplaceAll(trimmed, contextplane.ReferenceContextNote, "")
...
func formatToolStatePacket(state *contextplane.ToolLifecycleState) string {
	if state == nil {
		return ""
	}
	state.Mu().Lock()
	...
	if len(state.RecentResults) > 0 {
		...
		for _, record := range state.RecentResults {
			...
			lines = append(lines, strings.Join(contextplane.NonEmptyMemoryParts(parts), " "))
		}
		sections = append(sections, strings.Join(lines, "\n"))
	}
...
func sortedLoadedToolNamesLocked(state *contextplane.ToolLifecycleState) []string {
...
func sortedDeferredToolNamesLocked(state *contextplane.ToolLifecycleState) []string {
