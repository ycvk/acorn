...
func TestCompressionTokenCounterUsesConfiguredEncoding(t *testing.T) {
	...
	got, err := counter.count(context.Background(), messages, tools)
	if err != nil {
...
func TestCompressionTokenCounterRejectsUnknownEncoding(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "unknown_encoding") {
		t.Fatalf("NewCompressionTokenCounter error = %v, want unknown encoding", err)
	}
}

func TestCompressionPipelineOrdersStackBeforeCustomHandler(t *testing.T) {
	middlewares, err := NewCompressionPipeline().Build(context.Background(), compressionEnabledTestConfig(), &fakeCompressionChatModel{
...
func containsLayer(layers []CompactLayer, want CompactLayer) bool {
...
func TestContextCompressionPipelineAutoTrigger(t *testing.T) {
	...
	if !containsLayer(result.LayersApplied, CompactLayerAutocompact) {
...
func TestPreservedTailKeepsAssistantToolCallPair(t *testing.T) {
	...
	policy := PreservePolicy{RecentTurns: 1, PreserveToolPairs: true}
...
func TestCompressionIncrementalInputExcludesPriorSummaryFromNewTurns(t *testing.T) {
	...
	previousSummaryMsg := markCompressionSummary(schema.UserMessage(previousSummary))
	...
	}, PreservePolicy{RecentTurns: 1, PreserveToolPairs: true})
	if err != nil {
		t.Fatalf("buildSummarizerInput: %v", err)
	}
